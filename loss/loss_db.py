...
from loss.loss_basic import BalanceCrossEntropyLoss, MaskL1Loss, DiceLoss
...
class DBLoss(nn.Module):
    ...
    def forward(self, pred, batch):
        batch['shrink_map'] = torch.unsqueeze(torch.from_numpy(batch['shrink_map']), 0).cuda()
        batch['shrink_mask'] = torch.unsqueeze(torch.from_numpy(batch['shrink_mask']), 0).cuda()
        batch['threshold_map'] = torch.unsqueeze(torch.from_numpy(batch['threshold_map']), 0).cuda()
        batch['threshold_mask'] = torch.unsqueeze(torch.from_numpy(batch['threshold_mask']), 0).cuda()
